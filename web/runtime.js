...
function odinSetupDefaultImports(wasmMemoryInterface, consoleElement, memory) {
	...
	const env = {}
		...
		window.odin.webglContext = webglContext;
	...
	return {
		...
		//note(marco): Begin of custom owg bindings
		"owg": { 
		    load_image_data: (path_ptr, path_len, textureId) => {
				let path = wasmMemoryInterface.loadString(path_ptr, path_len);

				let image = document.createElement("img");
				image.id = "img_" + textureId;
				image.style.display = "none";
				document.body.append(image);

				const gl = webglContext.ctx;

			    
			    image.onload = () => {
					webglContext.getWebGL1Interface().BindTexture(gl.TEXTURE_2D, textureId)
			      	gl.texImage2D(
			        	gl.TEXTURE_2D,
			        	0,
			        	gl.RGBA,
		    ...
		    get_image_size: (image_id_ptr, image_id_len, w_ptr, h_ptr) => {
		    	let name = wasmMemoryInterface.loadString(image_id_ptr, image_id_len)
		    	let element = document.getElementById(name)
		    	if(element) {
		    		wasmMemoryInterface.storeI32(w_ptr, element.width)
		    		wasmMemoryInterface.storeI32(h_ptr, element.height)
		    	} else {
		    		console.log("element not found", name)
		    	}
		    },
			
			set_element_size: (id_ptr, id_len, w, h) => {
				const id = wasmMemoryInterface.loadString(id_ptr, id_len);
